...
async function myScript() {
  // Added code to fetch different JSON based on host
  const url = new URL(window.location.href);
  const host = url.hostname;
  let jsonData;

  if (host === 'https://robertsspaceindustries.com/orgs/*') {
    jsonData = await fetchJsonFromStorage('https://cdn.jsdelivr.net/gh/CxJuice/Uex_Chinese_Translate@main/RSI-WEB-ST/orgs1.3.json');
  } else if (host === 'https://robertsspaceindustries.com/account/concierge/*') {
    jsonData = await fetchJsonFromStorage('https://cdn.jsdelivr.net/gh/CxJuice/Uex_Chinese_Translate@main/RSI-WEB-ST/concierge1.1.json');
  } else {
    jsonData = await fetchJsonFromStorage('https://cdn.jsdelivr.net/gh/CxJuice/Uex_Chinese_Translate@main/zh-CN-rsi0.2.11.json');
  }
  ...
  async function getLocales(lang) {
    ...
    if(SUPPORT_LANG.includes(lang)) {
      const response = await fetch('https://cdn.jsdelivr.net/gh/CxJuice/Uex_Chinese_Translate@main/zh-CN-rsi0.2.11.json');
...
async function fetchJsonFromStorage(key) {
...
// 根据 URL 值执行不同功能
//__ORGS__CITIZENS__
if (block2.test(url)||block3.test(url)||url === "https://robertsspaceindustries.com/account/organization")
{
...
{
//__REFERRAL__
if (url === "https://robertsspaceindustries.com/account/referral-program" ||"https://robertsspaceindustries.com/account/referral-program?recruits=1")
{
...
//__HANGAR__
if (block5.test(url)||block6.test(url)||block7.test(url)||block8.test(url))
{
